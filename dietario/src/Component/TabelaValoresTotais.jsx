...
const TabelaValoresTotais = (props) => {
    ...
    { field: "nome", headerName: "Nutriente", width: 130 },
    {
      ...
      cellClassName: (params) =>
        clsx("super-app", {
          ...
          neutral:
            0.3 <= params.getValue("selecionado") / params.getValue("recomendado") &&
            params.getValue("selecionado") / params.getValue("recomendado") <= 0.7,
  ...
  useEffect(() => {
    setFoodsInDiet(props.foods.filter((item) => props.diet.includes(item.id)));
  }, [props]);
  useEffect(() => {
  ...
  const getSum = (items) => {
    ...
    keys.forEach((key, index) => {
      result[key] = items.map((el) => isNumerical(el[key])).reduce((acc, int) => acc + int);
    });
    delete result["Nome"];
    delete result["id"];
    return result;
  ...
  return (
    <>
      <TypoGraphy className={classes.title} variant="h3" component="h2" style={{ marginBottom: -13, marginTop: 44 }}>
        Valores Diários
      </TypoGraphy>
      <DataGrid
        rowHeight={32}
        ...
        rows={valoresDiarios.map((item) => formatRowData(item))}
        columns={columns}
      />
    </>
  );
